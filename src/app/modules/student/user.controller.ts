...
const createUser = async (req: Request, res: Response) => {
  try {
    ...
    const { password, _id, ...userWithoutPassword } = result.toObject();
...
const updateUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { password, _id, orders, ...userWithoutPassword } = result.toObject();
    userWithoutPassword.fullName = {
      firstName: userWithoutPassword.fullName.firstName,
      lastName: userWithoutPassword.fullName.lastName,
    };
...
const createUserOrder = async (req: Request, res: Response) => {
  try {
    ...
    await UserServices.createUserOrderService(userId, zodParsedData);
    res.status(200).json({
      success: true,
